...
impl MetaStore for DatabaseMetaStore {
    ...
    async fn replace_slices_for_compact_with_version(
        ...
        let delayed_slices = SliceDesc::decode_delayed_data(old_slices_to_delay)
            .ok_or_else(|| MetaError::Internal("Invalid delayed data length".to_string()))?;
        let expected_slice_ids: HashSet<i64> = delayed_slices
            .iter()
            .map(|(slice_id, _, _)| *slice_id as i64)
            .collect();
        ...
        let current_slices: Vec<slice_meta::Model> = SliceMeta::find()
            ...
            .await
            .map_err(MetaError::Database)?;
        let current_slice_ids: HashSet<i64> = current_slices.iter().map(|s| s.slice_id).collect();

        if current_slices.len() != expected_slice_count || current_slice_ids != expected_slice_ids {
            warn!(
                chunk_id = chunk_id,
                expected = expected_slice_count,
                actual = current_slices.len(),
                "Concurrent modification detected during compaction"
            );
            txn.rollback().await.map_err(MetaError::Database)?;
            return Err(MetaError::ContinueRetry);
        }
